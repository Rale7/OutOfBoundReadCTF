...
std::string Level8Service::getAccounts() {
  std::string result = "[";
  for (int i = 0; i < 3; ++i) {
...
std::string Level8Service::getAccount(int index) {
  if (index >= 3 || index < 0) {
    return "[]";
  }
...
void Level8Service::changeAccount(const Account1& newAccount) {
...
std::string Level8Service::exportFromFile(size_t size) {
  char guard[4096] = {0};
  char buffer[256] = {0};

  FILE* file = fopen("level8_export.json", "r");
  if (!file) {
...
void Level8Service::importToFile(const char data[], size_t size) {
  memcpy(buffer, data, 256);
  FILE* file = fopen("level8_export.json", "w");
  if (!file) {
    return;
  }
